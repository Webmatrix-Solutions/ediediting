...
function hide_pwd_reset_link($text) {
    ...
    if (get_option('hide_pwd_reset_link') == '1') {
        ...
        function remove_lostpassword_word ( $text ) {
            if ($text == 'Lost your password?') {$text = '';}
                   return $text;
            }
        
        function disable_password_reset_function(){
            return false;
        }
      
        add_filter('allow_password_reset', 'disable_password_reset_function');    
        add_filter( 'gettext', 'remove_lostpassword_word' );
